...
module.exports.solution = (input) => {
  ...
  return combinations(containersVolumes)
    .filter((combination) => {
      const valid = combination.reduce((a, b) => a + b) === liters
      if (valid && combination.length < minimum) {
        minimum = combination.length
      }
      return valid
    })
    .filter(combination => combination.length === minimum)
    .length
}

module.exports.expectedResult = 4
